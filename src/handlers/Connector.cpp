...
void Connector::handleWrite()
{
	...
	socket::SocketError err = m_socket->getError();
	if (err == socket::SocketError::E_TIMEOUT || err == socket::SocketError::E_CONNREFUSED)
	{
		// TODO: should not use ::strerror() here
		SPDLOG_LOGGER_ERROR(logger, "Connector error {}", ::strerror(errno));
		if (!m_retryTimer)
			m_retryTimer = make_unique<Timer>(EventLoop::thisLoop());
		reconnect();
	}
	else if (err == socket::SocketError::E_INPROGRESS)
	{
