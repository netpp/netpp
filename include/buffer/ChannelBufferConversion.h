...
public:
	...
	virtual std::unique_ptr<BufferIOConversion> readBufferConvert(Channel *channel) = 0;

	virtual std::unique_ptr<BufferIOConversion> writeBufferConvert(Channel *channel) = 0;
};
...
public:
	...
	*/
	std::unique_ptr<BufferIOConversion> readBufferConvert(Channel *channel) override;

	/**
	* @brief Write peer message to buffer
	* @return The ByteArray to iovec convertor
	*/
	std::unique_ptr<BufferIOConversion> writeBufferConvert(Channel *channel) override;
};
